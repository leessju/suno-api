...
// PATCH /api/music-gen/youtube-clips/[id]
// body: { title?, description?, thumbnail_key?, status?, deleted? }
export async function PATCH(req: NextRequest, { params }: Params) {
  try {
    ...
    if (body.deleted === true) {
      db.prepare('UPDATE youtube_clips SET deleted_at = ?, updated_at = ? WHERE id = ?')
        .run(now, now, id)
      return ok({ id })
    }
    ...
    if (sets.length > 1) {
      db.prepare(`UPDATE youtube_clips SET ${sets.join(', ')} WHERE id = ?`)
        .run(...vals, id)
    }
