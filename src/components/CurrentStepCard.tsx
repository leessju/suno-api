...
const STEP_INFO: Record<number, { title: string; desc: string; href?: string; action?: string }> = {
  ...
  3: { title: '원곡 파일을 업로드하세요', desc: '원곡을 업로드하면 분석 후 Cover곡을 생성할 수 있습니다.' },
  4: { title: 'Cover곡을 선택하세요', desc: '파이프라인에 포함할 Cover곡을 체크합니다.', href: 'variants', action: 'Cover곡 선택' },
  5: { title: '각 Cover곡에 이미지를 연결하세요', desc: '썸네일/커버 이미지를 업로드하거나 연결합니다.', href: 'images', action: '이미지 연결' },
  6: { title: '렌더링을 실행하세요', desc: 'Remotion Job을 통해 영상을 생성합니다.' },
  7: { title: '머지 순서를 설정하세요', desc: '최종 영상에서 Cover곡을 합칠 순서를 지정합니다.', href: 'merge', action: '순서 지정' },
